...
def test_initial_state(monitor) -> None:
    """Test that monitor initializes with correct defaults."""
    assert monitor.interval == pytest.approx(1.0)
    assert monitor.history == []
    assert all(value == pytest.approx(0.0) for value in monitor.peak_usage.values())
    assert monitor._disk_before is None
    assert monitor._net_before is None


def test_collect_metrics_basic(monkeypatch, monitor) -> None:
    ...
    assert metrics["disk_read_mb"] == pytest.approx(0.0)
    assert metrics["disk_write_mb"] == pytest.approx(0.0)
    assert metrics["network_up_mb"] == pytest.approx(0.0)
    assert metrics["network_down_mb"] == pytest.approx(0.0)


def test_collect_metrics_with_previous_values(monkeypatch) -> None:
...
def test_update_and_peak_usage(monitor) -> None:
    ...
    assert monitor.peak_usage["cpu_percent"] == pytest.approx(80.0)
    assert monitor.peak_usage["memory_percent"] == pytest.approx(70.0)
    assert monitor.peak_usage["memory_used_gb"] == pytest.approx(12.0)
    assert monitor.peak_usage["disk_percent"] == pytest.approx(30.0)
    assert monitor.peak_usage["disk_used_gb"] == pytest.approx(150.0)
    assert monitor.peak_usage["disk_read_mb"] == pytest.approx(5.0)
    assert monitor.peak_usage["network_up_mb"] == pytest.approx(2.0)

    assert len(monitor.history) == 2


def test_sample_adds_history(monkeypatch, monitor) -> None:
...
def test_get_summary(monitor) -> None:
...
def test_get_summary_empty_history(monitor) -> None:
    """Test get_summary() with empty history."""
    assert monitor.get_summary() == {}


def test_get_peak_usage(monitor) -> None:
    """Test get_peak_usage() returns a copy."""
    monitor.peak_usage["cpu_percent"] = 90.0
    peaks = monitor.get_peak_usage()

    assert peaks["cpu_percent"] == pytest.approx(90.0)
    peaks["cpu_percent"] = 0.0
    assert monitor.peak_usage["cpu_percent"] == pytest.approx(90.0)


def test_get_history(monitor) -> None:
    """Test get_history() returns stored history."""
    monitor.history = [{"cpu_percent": 10.0}, {"cpu_percent": 20.0}]
    history = monitor.get_history()

    assert len(history) == 2
    assert history[0]["cpu_percent"] == pytest.approx(10.0)


def test_clear_history_resets_state(monitor) -> None:
...
def test_monitor_with_duration(monitor) -> None:
...
def test_monitor_keyboard_interrupt(monitor) -> None:
...
def test_get_recent_alerts(monitor) -> None:
...
def test_get_recommendations(monitor) -> None:
...
def test_get_stats(monitor) -> None:
...
def test_check_alerts(monitor) -> None:
