...
class LiveMonitorUI:
    ...
    def _render(self) -> Panel:
        ...
        # CPU
        text.append(f"CPU:  {MonitorUI.create_progress_bar(cpu)} {cpu:.0f}%\n")

        # RAM - add check for zero division
        if ram_total > 0:
            text.append(
                text.append(
                    f"RAM:  {MonitorUI.create_progress_bar(ram_percent)} "
                    f"{ram_used:.1f}/{ram_total:.1f} GB ({ram_percent:.0f}%)\n"
                )
            )
        else:
        ...
        if disk_total > 0:
            text.append(
                text.append(
                    f"Disk: {MonitorUI.create_progress_bar(disk_percent)} "
                    f"{disk_used:.1f}/{disk_total:.1f} GB ({disk_percent:.0f}%)\n"
                )
            )
        else:
            ...
            with Live(self._render(), refresh_per_second=4, screen=False) as live:
                while not self._stop_event.is_set():
                    try:
                        ...
                        logger.error("UI rendering error: %s", exc)
                        time.sleep(0.5)
